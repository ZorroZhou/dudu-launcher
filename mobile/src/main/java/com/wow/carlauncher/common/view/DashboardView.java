...
public class DashboardView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        String mHeaderText = "km/h";
        for (int i = 0; i <= mSection; i++) {
            a = mStartAngle + angle * i;
            p = getCoordinatePoint(mRadius - mLength2, a);
            if (a % 360 > 135 && a % 360 < 225) {
                mPaint.setTextAlign(Paint.Align.LEFT);
            } else if ((a % 360 >= 0 && a % 360 < 45) || (a % 360 > 315 && a % 360 <= 360)) {
        ...
        //画实时度数值
        mPaint.setColor(ContextCompat.getColor(getContext(), R.color.plugin_content));
